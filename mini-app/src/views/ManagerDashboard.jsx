...
const ManagerDashboard = ({ user, activeTab, setActiveTab, onLogout }) => {
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#000', color: '#fff', paddingBottom: '110px', fontFamily: '"Outfit", sans-serif', overflowX: 'hidden' }}>
            ...
            <AnimatePresence mode="wait">
                ...
                {activeTab === 'rooms' && selectedViewRoom && (
                    <motion.div key="room-detail" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} style={{ padding: '15px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px' }}><button onClick={() => setSelectedViewRoom(null)} style={{ background: '#0a0a0a', width: '42px', height: '42px', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.05)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ArrowLeft size={20} /></button><h1 style={{ margin: 0, fontSize: '22px', fontWeight: '950' }}>{rooms.find(r => r.id === selectedViewRoom.id)?.name?.toUpperCase()}</h1></div>
            ...
            <AnimatePresence>
                {selectedPC && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 2000, display: 'flex', alignItems: 'flex-end', backdropFilter: 'blur(40px)' }} onClick={() => setSelectedPC(null)}>
                        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} style={{ background: '#0a0a0a', width: '100%', padding: '30px 25px 60px', borderRadius: '50px 50px 0 0', borderTop: '1px solid rgba(112, 0, 255, 0.3)', boxShadow: '0 -20px 60px rgba(0,0,0,0.8)' }} onClick={e => e.stopPropagation()}>

                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
                                <div><h1 style={{ margin: 0, fontSize: '28px', fontWeight: '950' }}>{selectedPC.name}</h1><p style={{ color: '#444', fontSize: '11px', fontWeight: '900' }}>{(selectedPC.specs || 'PREMIUM PC').toUpperCase()}</p></div>
                                <div style={{ textAlign: 'right', display: 'flex', gap: '15px', alignItems: 'center' }}>
                                    <div style={{ textAlign: 'right' }}><p style={{ fontSize: '8px', color: '#333', margin: 0, fontWeight: '900' }}>REAL TIME</p><b style={{ fontSize: '14px' }}>{formatTashkentTime(new Date(nowTime))}</b></div>
                                    <button onClick={() => setSelectedPC(null)} style={{ background: '#151515', width: '42px', height: '42px', borderRadius: '15px', border: 'none', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={20} /></button>
                                </div>
                            </div>
                            ...
                            {(() => {
                                ...
                                return (
                                    <div style={{ marginBottom: '35px' }}>
                                        {s !== 'free' && s !== 'reserved' && (
                                            <div style={{ textAlign: 'center', background: 'rgba(255,255,255,0.02)', padding: '35px 20px', borderRadius: '40px', border: '1px solid rgba(255,255,255,0.04)' }}>
                                                <p style={{ fontSize: '10px', color: '#444', fontWeight: '950', letterSpacing: '4px' }}>TIME REMAINING</p>
                                                <h1 style={{ fontSize: '86px', color: themeColor, fontWeight: '950', margin: '5px 0', letterSpacing: '-4px' }}>{info.time}</h1>
                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px', marginTop: '20px' }}>
                                                    <div><p style={{ fontSize: '9px', color: '#333', margin: 0, fontWeight: '900' }}>BOSHLAHDY</p><b style={{ fontSize: '15px' }}>{info.startTime || '--:--'}</b></div>
                                                    <div style={{ width: '1px', background: '#1a1a1a', height: '30px', margin: '5px auto' }} />
                                                    <div><p style={{ fontSize: '9px', color: '#333', margin: 0, fontWeight: '900' }}>HISOBLAHDY</p><b style={{ fontSize: '22px', color: '#fff' }}>{info.cost?.toLocaleString()}</b></div>
                                                </div>
                                            </div>
                                        )}
                                        {s === 'reserved' && (
                                            <div style={{ background: 'rgba(255, 170, 0, 0.05)', padding: '40px', borderRadius: '45px', border: '1px solid rgba(255,170,0,0.2)', textAlign: 'center' }}>
                                                <UserIcon color="#ffaa00" size={40} style={{ marginBottom: '15px' }} />
                                                <h3 style={{ fontSize: '28px', margin: 0, fontWeight: '950' }}>{info.reservedInfo?.user}</h3>
                                                <b style={{ fontSize: '20px', color: '#ffaa00' }}>{formatTashkentTime(info.reservedInfo?.time)}</b>
                                            </div>
                                        )}
                                        {s === 'free' && (
                                            <div style={{ width: '100%' }}>
                                                ...
                                                <div style={{ position: 'relative', marginBottom: '20px' }}>
                                                    <Banknote style={{ position: 'absolute', left: '25px', top: '50%', transform: 'translateY(-50%)', color: '#222' }} size={28} />
                                                    <input type="number" placeholder="SUMMA" value={startAmountInput} onChange={e => setStartAmountInput(e.target.value)} style={{ width: '100%', padding: '30px 20px 30px 65px', background: '#000', border: '1px solid #1a1a1a', borderRadius: '30px', color: '#39ff14', fontSize: '32px', fontWeight: '950', textAlign: 'left' }} />
                                                    <div style={{ position: 'absolute', right: '25px', top: '50%', transform: 'translateY(-50%)', color: '#333', fontWeight: '950' }}>UZS</div>
                                                </div>

                                                {/* TOUCH-FRIENDLY QUICK ACTIONS */}
                                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '25px' }}>
                                                    <QuickTouchBtn label="10.000 UZS" sub="10K" onClick={() => setStartAmountInput('10000')} />
                                                    <QuickTouchBtn label="20.000 UZS" sub="20K" onClick={() => setStartAmountInput('20000')} />
                                                    <QuickTouchBtn label="30.000 UZS" sub="30K" onClick={() => setStartAmountInput('30000')} />
                                                    <QuickTouchBtn label="50.000 UZS" sub="50K" onClick={() => setStartAmountInput('50000')} />

                                                    <QuickTouchBtn label="1 SOAT" sub="TIME" color="#7000ff" onClick={() => handleAction('start', 60)} />
                                                    <QuickTouchBtn label="2 SOAT" sub="TIME" color="#7000ff" onClick={() => handleAction('start', 120)} />
                                                    <QuickTouchBtn label="5 SOAT" sub="TIME" color="#7000ff" onClick={() => handleAction('start', 300)} />
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                {selectedPC.status !== 'free' && <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleAction('stop')} style={{ padding: '25px', borderRadius: '25px', background: 'rgba(255, 68, 68, 0.1)', color: '#ff4444', border: 'none', fontWeight: '950', fontSize: '16px', letterSpacing: '1px' }}>STOP ⏹️</motion.button>}
                                <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleAction(selectedPC.status === 'busy' ? 'pause' : 'start')} style={{ padding: '25px', borderRadius: '25px', background: selectedPC.status === 'busy' ? '#ffee32' : 'linear-gradient(45deg, #7000ff, #a000ff)', color: selectedPC.status === 'busy' ? '#000' : '#fff', border: 'none', fontWeight: '950', fontSize: '18px', gridColumn: (selectedPC.status === 'free' || selectedPC.status === 'reserved') ? 'span 2' : 'auto', boxShadow: selectedPC.status === 'free' ? '0 15px 40px rgba(112, 0, 255, 0.4)' : 'none' }}>{selectedPC.status === 'busy' ? 'PAUZA' : 'VAQTNI OCHISH 🚀'}</motion.button>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
...
// 🛠️ NEW TOUCH-FRIENDLY BUTTON COMPONENT
const QuickTouchBtn = ({ label, sub, onClick, color = '#666' }) => (
    <motion.button
        whileTap={{ scale: 0.94 }}
        onClick={onClick}
        style={{
            background: 'rgba(255,255,255,0.03)',
            border: `1px solid ${color === '#666' ? 'rgba(255,255,255,0.05)' : color + '30'}`,
            borderRadius: '22px',
            padding: '18px 10px',
            display: 'flex',
            flexDirection: 'column',
    ...
    >
        <span style={{ fontSize: '9px', fontWeight: '950', color: '#333', letterSpacing: '1px' }}>{sub}</span>
        <span style={{ fontSize: '14px', fontWeight: '950', color }}>{label}</span>
    </motion.button>
);
