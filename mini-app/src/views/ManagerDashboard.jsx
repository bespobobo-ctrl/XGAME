                        ...
                        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} style={{ background: '#111', width: '100%', padding: '35px 25px', borderRadius: '45px 45px 0 0', borderTop: '1px solid #1a1a1a' }}>
                            ...
                            {!showReservePicker ? (
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                    {((selectedPC.status || '').toLowerCase() === 'free' || (selectedPC.status || '').toLowerCase() === 'off') && (
                                        <><div style={{ gridColumn: 'span 2', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                                            ...
                                            <div style={{ gridColumn: 'span 2', display: 'flex', gap: '12px' }}><input type="number" placeholder="Da..." value={customMinutes} onChange={e => setCustomMinutes(e.target.value)} style={{ flex: 1.5, background: '#000', border: '1.5px solid #1a1a1a', borderRadius: '22px', padding: '18px', color: '#fff', textAlign: 'center', fontSize: '18px' }} /><button onClick={() => handleAction('start', parseInt(customMinutes))} disabled={!customMinutes || actionLoading} style={{ flex: 1, background: '#7000ff', color: '#fff', borderRadius: '22px', fontWeight: '900' }}>START</button></div>
                                            <button onClick={() => setShowReservePicker(true)} disabled={actionLoading} style={{ gridColumn: 'span 2', padding: '22px', borderRadius: '25px', background: '#ffaa0015', color: '#ffaa00', border: '1.5px solid #ffaa0044', fontWeight: '950' }}>🕒 BRON</button></>
                                    )}
                                    {((selectedPC.status || '').toLowerCase() === 'busy' || (selectedPC.status || '').toLowerCase() === 'paused') && (
                                ...
                                <div style={{ background: '#000', padding: '30px', borderRadius: '40px', border: '1px solid #1a1a1a' }}>
                                    <h3 style={{ margin: '0 0 20px', textAlign: 'center' }}>BRON</h3>
                                    <input placeholder="Ism" value={reserveNameInput} onChange={e => setReserveNameInput(e.target.value)} style={{ width: '100%', padding: '20px', background: '#111', border: '1.5px solid #1a1a1a', borderRadius: '22px', color: '#fff', marginBottom: '10px' }} />
                                    <input type="time" value={reserveTimeInput} onChange={e => setReserveTimeInput(e.target.value)} style={{ width: '100%', padding: '20px', background: '#111', border: '1.5px solid #1a1a1a', borderRadius: '22px', color: '#fff', marginBottom: '20px', fontSize: '28px', textAlign: 'center' }} />
