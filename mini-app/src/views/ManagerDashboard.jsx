...
const ManagerDashboard = ({ user, activeTab, setActiveTab, onLogout }) => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (selectedPC && freshRooms.length > 0) {
    ...
    useEffect(() => {
        fetchData();
        const dataInterval = setInterval(fetchData, 3000); // 3s for better reactivity
    ...
    const handleAction = async (action, expectedMinutes = null, reserveTime = null, guestName = null) => {
        ...
        try {
            ...
            if (res.success) {
                await fetchData(); // 🔥 AWAIT FRESH DATA
                setShowReservePicker(false);
                setCustomMinutes('');
                // If stop, maybe close modal? Users usually want it closed after stop.
    ...
    const handleApproveTopup = async (id, action, amount) => {
        ...
        try {
            ...
            if (res.success) { await fetchData(); } else alert(res.error);
        } catch (e) { alert(e.message); }
    };
    ...
    const renderPC = (pc, room) => {
        ...
        const getStatusTheme = () => {
            if (status === 'free' || status === 'available') return { color: '#39ff14', icon: <Monitor size={28} />, label: 'BO\'SH' };
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'stats' && (
                    <motion.div key="stats" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} style={{ padding: '20px' }}>
                        ...
                        {stats?.urgentReservations?.length > 0 && (
                            <div style={{ background: 'linear-gradient(90deg, #ffaa00, #ff4444)', padding: '15px', borderRadius: '25px', marginBottom: '20px', border: '1px solid #0002' }}>
                                <h4 style={{ margin: '0 0 5px', fontSize: '12px', color: '#000', fontWeight: 'bold', textAlign: 'center' }}>DIQQAT: BRON VAQTI KELDI!</h4>
                                {stats.urgentReservations.map((ur, i) => (
                                    <p key={i} style={{ margin: 0, fontSize: '11px', color: '#000', textAlign: 'center' }}>{ur.pc} - {ur.user} ({ur.phone || "TEL YO'Q"})</p>
                                ))}
                            </div>
                        )}
                        ...
                        <div style={{ display: 'grid', gap: '12px' }}>
                            {['day', 'week', 'month', 'year'].map(period => (
                                <div key={period} style={{ background: 'linear-gradient(90deg, #111, #0a0a0a)', border: '1px solid #1a1a1a', padding: '18px 25px', borderRadius: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                        <h5 style={{ margin: 0, fontSize: '10px', color: '#444', letterSpacing: '1px' }}>{period.toUpperCase()}</h5>
                ...
                {activeTab === 'rooms' && selectedViewRoom && (
                    <motion.div key="room-detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} style={{ padding: '20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '25px' }}>
                            ...
                            <div><h2 style={{ margin: 0, fontSize: '22px', fontWeight: '900' }}>{selectedViewRoom.name.toUpperCase()}</h2><p style={{ margin: 0, fontSize: '11px', color: '#7000ff', fontWeight: 'bold' }}>{selectedViewRoom.Computers?.length} TA KOMPYUTER</p></div>
                        </div>
                ...
                {activeTab === 'payments' && (
                    <motion.div key="payments" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: '20px' }}>
                        ...
                        {topupRequests.length === 0 ? <p style={{ textAlign: 'center', opacity: 0.2, marginTop: '100px' }}>Hozircha yo'q</p> :
                            topupRequests.map(req => (
                                <div key={req.id} style={{ background: '#111', borderRadius: '35px', padding: '25px', marginBottom: '15px', border: '1px solid #1a1a1a' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '12px' }}>
                                        <button onClick={() => handleApproveTopup(req.id, 'reject')} style={{ flex: 1, padding: '18px', borderRadius: '18px', background: '#ff444415', color: '#ff4444' }}>RAD</button>
                                        <button onClick={() => handleApproveTopup(req.id, 'approve')} style={{ flex: 1.5, padding: '18px', borderRadius: '18px', background: '#39ff14', color: '#000', fontWeight: '900' }}>OK</button>
                                    </div>
                                </div>
                            ))
                        }
                ...
                {activeTab === 'settings' && (
                    <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: '20px' }}>
                        <div style={{ background: '#111', padding: '30px', borderRadius: '40px', border: '1px solid #222' }}>
                            <h4 style={{ margin: '0 0 25px', fontSize: '12px', color: '#7000ff', fontWeight: 'bold' }}>KARTA SOZLAMALARI</h4>
                        ...
                        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} style={{ background: '#111', width: '100%', padding: '35px 25px', borderRadius: '45px 45px 0 0', borderTop: '1px solid #1a1a1a' }}>
                            ...
                            {!showReservePicker ? (
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                    {((selectedPC.status || '').toLowerCase() === 'free' || (selectedPC.status || '').toLowerCase() === 'available' || (selectedPC.status || '').toLowerCase() === 'off') && (
                                        <><div style={{ gridColumn: 'span 2', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                                            ...
                                            <div style={{ gridColumn: 'span 2', display: 'flex', gap: '12px' }}><input type="number" placeholder="Da..." value={customMinutes} onChange={e => setCustomMinutes(e.target.value)} style={{ flex: 1.5, background: '#000', border: '1.5px solid #1a1a1a', borderRadius: '22px', padding: '18px', color: '#fff', textAlign: 'center', fontSize: '18px' }} /><button onClick={() => handleAction('start', parseInt(customMinutes))} disabled={!customMinutes || actionLoading} style={{ flex: 1, background: '#7000ff', color: '#fff', borderRadius: '22px', fontWeight: '900' }}>OK</button></div>
                                ...
                                <div style={{ background: '#000', padding: '30px', borderRadius: '40px', border: '1px solid #1a1a1a' }}>
                                    <h3 style={{ margin: '0 0 20px', textAlign: 'center', fontWeight: '900' }}>BRON</h3>
                                    <input placeholder="Ism" value={reserveNameInput} onChange={e => setReserveNameInput(e.target.value)} style={{ width: '100%', padding: '20px', background: '#111', border: '1.5px solid #1a1a1a', borderRadius: '22px', color: '#fff', marginBottom: '12px' }} />
                                    <input type="time" value={reserveTimeInput} onChange={e => setReserveTimeInput(e.target.value)} style={{ width: '100%', padding: '20px', background: '#111', border: '1.5px solid #1a1a1a', borderRadius: '22px', color: '#fff', marginBottom: '25px', fontSize: '28px', textAlign: 'center' }} />
                                    <div style={{ display: 'flex', gap: '15px' }}><button onClick={() => setShowReservePicker(false)} style={{ flex: 1, padding: '20px', borderRadius: '22px', background: '#1c1c1e', color: '#fff' }}>BEKOR</button><button onClick={() => handleAction('reserve', null, reserveTimeInput, reserveNameInput)} disabled={actionLoading} style={{ flex: 1.5, padding: '20px', borderRadius: '22px', background: '#ffaa00', color: '#000', fontWeight: '950' }}>SAQLASH</button></div>
                                </div>
                            )}
                        </motion.div>
                    </motion.div>
