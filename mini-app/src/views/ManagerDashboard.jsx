    ...
    return (
        <div style={{ minHeight: '100vh', background: '#000', color: '#fff', paddingBottom: '120px', fontFamily: '"Outfit", sans-serif', overflowX: 'hidden' }}>
            <style>
                ...
                .premium-box { background: rgba(255,255,255,0.065); backdrop-filter: blur(50px); border: 1px solid rgba(255,255,255,0.15); border-radius: 55px; box-shadow: 0 40px 100px rgba(0,0,0,0.7); overflow: hidden; }
            ...
            <AnimatePresence mode="wait">
                ...
                {activeTab === 'rooms' && !selectedViewRoom && (
                    <motion.div key="rooms" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: '15px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '25px', alignItems: 'center' }}><h1 style={{ fontSize: '22px', fontWeight: '950', margin: 0, color: '#fff' }}>XONALAR</h1></div>
                        {rooms.map(room => {
            ...
            {/* 🚀 MODAL: PRO CLUBTIMER VERSION 2026 */}
            <AnimatePresence>
                {selectedPC && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.96)', zIndex: 2000, display: 'flex', alignItems: 'flex-end', backdropFilter: 'blur(30px)' }} onClick={() => { setSelectedPC(null); setIsReserveMode(false); }}>
                        <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} style={{ background: '#050505', width: '100%', padding: '30px 20px 60px', borderRadius: '55px 55px 0 0', borderTop: '2px solid #7000ff', boxSizing: 'border-box' }} onClick={e => e.stopPropagation()}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '35px' }}>
                                <div><h1 style={{ margin: 0, fontSize: '38px', fontWeight: '950', letterSpacing: '-1.5px', color: '#fff' }}>{selectedPC.name}</h1><p className="secondary-label" style={{ fontSize: '11px', letterSpacing: '1.5px' }}>NEXUS COMMAND CENTER</p></div>
                                <button onClick={() => { setSelectedPC(null); setIsReserveMode(false); }} style={{ background: 'rgba(255,255,255,0.1)', width: '50px', height: '50px', borderRadius: '18px', border: 'none', color: '#fff' }}><X size={26} /></button>
                            </div>

                            {isReserveMode ? (
                                <div key="reserve-form">
                                    <div style={{ position: 'relative' }}> <Contact2 style={{ position: 'absolute', left: '20px', top: '50%', transform: 'translateY(-50%)', color: '#7000ff' }} size={22} /> <input className="res-input" placeholder="Mijoz ismi" value={resName} onChange={e => setResName(e.target.value)} /> </div>
                                    <div style={{ position: 'relative' }}> <Phone style={{ position: 'absolute', left: '20px', top: '50%', transform: 'translateY(-50%)', color: '#7000ff' }} size={22} /> <input className="res-input" placeholder="Tel raqami" value={resPhone} onChange={e => setResPhone(e.target.value)} /> </div>
                                    <div style={{ position: 'relative' }}> <Clock style={{ position: 'absolute', left: '20px', top: '50%', transform: 'translateY(-50%)', color: '#7000ff' }} size={22} /> <input className="res-input" type="time" value={resTime} onChange={e => setResTime(e.target.value)} /> </div>
                                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleAction('reserve')} style={{ width: '100%', padding: '26px', borderRadius: '30px', background: 'linear-gradient(45deg, #ffaa00, #ff7700)', color: '#000', border: 'none', fontSize: '18px', fontWeight: '950' }}>SAVE RESERVATION 🔒</motion.button>
                                </div>
                            ) : (
                                <>
                                    {selectedPC.status === 'free' ? (
                                        <div key="free-ui">
                                            <div style={{ position: 'relative', marginBottom: '30px', background: '#000', borderRadius: '35px', padding: '40px 25px', border: '2px solid #151515' }}>
                                                <input type="number" placeholder="0" value={startAmountInput} onChange={e => setStartAmountInput(e.target.value)} style={{ width: '100%', background: 'transparent', border: 'none', color: '#39ff14', fontSize: '66px', fontWeight: '950', outline: 'none', textAlign: 'center', letterSpacing: '-2px' }} />
                                                <div className="secondary-label" style={{ textAlign: 'center', fontSize: '10px', letterSpacing: '4px' }}>ENTER TOP-UP SUMMA</div>
                                            </div>
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: '15px' }}>
                                                <motion.button whileTap={{ scale: 0.9 }} onClick={() => setIsReserveMode(true)} style={{ padding: '26px', borderRadius: '35px', background: 'rgba(255,170,0,0.1)', color: '#ffaa00', border: '1px solid #ffaa0030', fontWeight: '950', fontSize: '16px' }}>RESERVE 🗓️</motion.button>
                                                <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleAction('start')} style={{ padding: '26px', borderRadius: '35px', background: 'linear-gradient(45deg, #7000ff, #a000ff)', color: '#fff', border: 'none', fontWeight: '950', fontSize: '20px' }}>START 🚀</motion.button>
                                            </div>
                                        </div>
                                    ) : (
                                        <div key="busy-ui">
                                            {(() => {
                                                ...
                                                return (
                                                    <div className="premium-box" style={{ padding: '50px 10px', textAlign: 'center', marginBottom: '40px' }}>
                                                        <p className="secondary-label" style={{ fontSize: '11px', letterSpacing: '8px', marginBottom: '10px' }}>TIME {info.isCountdown ? 'LEFT' : 'PASSED'}</p>
                                                        <h1 style={{ fontSize: '82px', margin: 0, fontWeight: '950', color: '#fff', letterSpacing: '-4px', lineHeight: 1 }}>{info.time}</h1>
                                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '40px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '35px' }}>
                                                            <div><p className="secondary-label" style={{ fontSize: '10px', margin: '0 0 8px' }}>STARTED AT</p><b style={{ fontSize: '24px', color: '#fff' }}>{info.startTime || '--:--'}</b></div>
                                                            <div><p className="secondary-label" style={{ fontSize: '10px', margin: '0 0 8px' }}>CURRENT COST</p><b style={{ fontSize: '28px', color: '#39ff14' }}>{info.cost?.toLocaleString()} UZS</b></div>
                                                        </div>
                                                    </div>
                                                );
                                            })()}
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                                <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleAction(selectedPC.status === 'paused' ? 'resume' : 'pause')} style={{ padding: '30px', borderRadius: '40px', background: '#ffee32', color: '#000', border: 'none', fontWeight: '950', fontSize: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>{selectedPC.status === 'paused' ? <><Play size={22} fill="#000" /> RESUME</> : <><Pause size={22} fill="#000" /> PAUSE</>}</motion.button>
                                                <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleAction('stop')} style={{ padding: '30px', borderRadius: '40px', background: 'rgba(255,68,68,0.15)', color: '#ff4444', border: '1px solid rgba(255,68,68,0.3)', fontWeight: '950', fontSize: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}><Square size={22} fill="#ff4444" /> STOP</motion.button>
                                            </div>
                                        </div>
                                    )}
                                </>
                            )}
                            <div style={{ textAlign: 'center', marginTop: '35px', opacity: 0.3, fontSize: '10px', letterSpacing: '2px', color: '#fff' }}>@X_Game123_bot</div>
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <nav style={{ position: 'fixed', bottom: '25px', left: '20px', right: '20px', background: 'rgba(12,12,12,0.96)', backdropFilter: 'blur(40px)', padding: '15px 10px', borderRadius: '45px', display: 'flex', justifyContent: 'space-around', zIndex: 1000, border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 20px 50px rgba(0,0,0,1)' }}>
                ...
                {navItem('rooms', 'Xarita', <Monitor size={22} strokeWidth={2.5} />)}
                {navItem('users', 'Mijoz', <Users size={22} strokeWidth={2.5} />)}
...
const QuickTapBtn = ({ label, onClick, className }) => (
    <motion.button whileTap={{ scale: 0.94 }} onClick={onClick} style={{ background: '#0d0d0d', border: '1px solid #222', borderRadius: '30px', padding: '30px 5px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <span className={className} style={{ fontSize: '18px', fontWeight: '950' }}>{label}</span>
    </motion.button>
);
const DashItem = ({ label, icon, value }) => (<div><div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>{icon}<p className="secondary-label" style={{ fontSize: '10px', margin: 0, letterSpacing: '1.5px' }}>{label}</p></div><b style={{ fontSize: '22px', color: '#fff', fontWeight: '950' }}>{value}</b></div>);
const MiniStat = ({ color, count, label }) => (<div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><div style={{ width: '8px', height: '8px', background: color, borderRadius: '50%', boxShadow: `0 0 10px ${color}` }} /><div style={{ display: 'flex', flexDirection: 'column' }}><span style={{ fontSize: '12px', color: count > 0 ? color : '#333', fontWeight: '950' }}>{count}</span><span style={{ fontSize: '8px', color: '#444', fontWeight: '950' }}>{label}</span></div></div>);
